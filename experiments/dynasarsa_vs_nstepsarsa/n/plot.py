...
def compute_meansandstderrors_overruns(df, metric):
    ...
    df_wmeans = df.groupby(['method','n'])[metric].mean().to_frame(name = 'mean_'+ metric).reset_index()
    df_wsems = df.groupby(['method', 'n'])[metric].sem().to_frame(name = 'stderror_'+ metric).reset_index()
...
def keep_onerow_perrun(df):
    return df.groupby(['method','run', 'n']).first().reset_index()

def plot_solutionepisode_vs_n(df):
...
def plot_nrunsolved_vs_n(df):
    ...
    run_df = df.groupby(['method','run','n'])['solution_episode'].max().to_frame(name = 'max_solution_episode').reset_index()
    run_df['run_solved'] = np.where(run_df['max_solution_episode'] > -1, 1, 0)
    cnt_df = run_df.groupby(['method','n'])['run_solved'].sum().to_frame(name = 'nrun_solved').reset_index()
    
    x1= extract_axisvalues_bymethod(cnt_df,method1,'n')
    y1= extract_axisvalues_bymethod(cnt_df,method1,'nrun_solved')
    x2= extract_axisvalues_bymethod(cnt_df,method2,'n')
